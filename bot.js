 ...
 * 
 * Uso: node bot.js <ip> <port> <bots> <tiempo> [register_cmd] [mensajes] [intervalo_msg] [skin.png]
 *
 * Ejemplos:
 *   node bot.js 127.0.0.1 19132 5 0
 *   node bot.js 127.0.0.1 19132 5 60 "/register" "Hola!|Como-estas" 5 skin.png
 */
...
const SKIN_PATH     = process.argv[9]  || null;

// ─── Cargar nombres desde accounts.yml ────────────────────────────────────────
let NOMBRES_REALES = [];
let NOMBRES_POR_DEFECTO = [];

try {
  const accountsPath = path.join(__dirname, 'accounts.yml');
  ...
  for (const line of lines) {
    if (line.trim() === 'players:') {
      inPlayers = true;
      continue;
    }
    if (inPlayers && line.trim().startsWith('  ') && line.includes(':')) {
      const nombre = line.trim().split(':')[0];
      if (nombre && !nombre.startsWith('#') && nombre !== 'players') {
        NOMBRES_REALES.push(nombre);
      }
    }
    if (inPlayers && line.trim() === '') continue;
    if (inPlayers && !line.trim().startsWith('  ')) break;
  }
  
  console.log(`[Skin] Cargados ${NOMBRES_REALES.length} nombres desde accounts.yml`);
} catch(e) {
  console.log(`[Skin] No se pudo cargar accounts.yml: ${e.message}`);
}

// Nombres de respaldo si no se pudo cargar
if (NOMBRES_REALES.length === 0) {
  NOMBRES_REALES = [
  ...
  console.log(`[Skin] Usando ${NOMBRES_REALES.length} nombres por defecto`);
}

// ─── Cargar skin personalizada desde PNG ──────────────────────────────────────
let SKIN_DATA = null;
let SKIN_WIDTH = 64;
let SKIN_HEIGHT = 64;
...
function loadSkinFromPNG(pngPath) {
  try {
    ...
    SKIN_WIDTH = png.width;
    SKIN_HEIGHT = png.height;
    
    // Convertir a RGBA (64x64 o 64x32 o 128x128)
    const rgba = Buffer.alloc(SKIN_WIDTH * SKIN_HEIGHT * 4);
    
    for (let i = 0; i < png.data.length; i += 4) {
      rgba[i] = png.data[i];     // R
      rgba[i+1] = png.data[i+1]; // G
      rgba[i+2] = png.data[i+2]; // B
      rgba[i+3] = png.data[i+3]; // A
    }
    
    SKIN_DATA = rgba.toString('base64');
    console.log(`[Skin] Cargada skin PNG: ${SKIN_WIDTH}x${SKIN_HEIGHT} (${Math.round(pngData.length/1024)}KB)`);
    return true;
  } catch(e) {
    console.log(`[Skin] Error cargando PNG: ${e.message}`);
    return false;
  }
}

// Si no se pudo cargar PNG, usar skin de Steve por defecto
if (SKIN_PATH && fs.existsSync(SKIN_PATH)) {
  try {
    require.resolve('pngjs');
    loadSkinFromPNG(SKIN_PATH);
  } catch(e) {
    console.log(`[Skin] PNG support no disponible, instalando: npm install pngjs`);
    console.log(`[Skin] Usando skin Steve por defecto`);
    SKIN_DATA = null;
  }
}

// Skin de Steve por defecto (64x64)
if (!SKIN_DATA) {
  ...
  SKIN_DATA = buf.toString('base64');
  console.log(`[Skin] Usando skin Steve por defecto (64x64)`);
}
...
console.log(`[Master] Bots      : ${BOTS} (nombres desde accounts.yml)`);
console.log(`[Master] Tiempo    : ${TIEMPO > 0 ? TIEMPO+'s' : 'ilimitado'}`);
console.log(`[Master] AntiBot   : Activado (comportamiento humano simulado)`);
console.log(`[Master] Skin      : ${SKIN_PATH ? (SKIN_DATA ? 'OK' : 'Steve por defecto') : 'Steve por defecto'}`);
...
// ─── Función para obtener nombre aleatorio (solo de accounts.yml) ────────────
function randomName() {
...
function buildLogin70(bot) {
  const skinBuf = Buffer.from(SKIN_DATA, 'base64');
...
function fullReconnect(bot) {
  ...
  sock.on('message', (msg) => {
    handleSocketMessage(bot, msg, sock);
  });
  
  // Bind con delay aleatorio
  setTimeout(() => {
    sock.bind(0, () => {
...
// ─── Request2 con múltiples variantes y simulación de cliente oficial ─────────
function sendRequest2WithRetry(bot) {
  ...
  const doSend = () => {
    ...
    // Enviar 3 copias para asegurar entrega
    for (let i = 0; i < 3; i++) {
      setTimeout(() => {
...
function iniciarConexion(bot, sock) {
  ...
  // Enviar múltiples pings con diferentes formatos
  const pings = [
  ...
  const sendNextPing = () => {
    ...
    if (pingCount < maxPings) {
      ...
      // Si no hubo respuesta después de todos los pings, intentar directo
...
function iniciarBot(numero) {
  // Usar nombre aleatorio de la lista (NUNCA base)
  const bot = {
    id: numero,
    nombre: randomName(),
    uuid: randomUUID(),
